...
const summaryData = {
  "Problem Space":
    "Companies are increasingly vulnerable to cyber threats. New exploits and vulnerabilities appear daily, creating an urgent need for advanced research and solutions.",
  "Research Methods":
    "OpenQuanta connects researchers worldwide, enabling them to collaborate, share methodologies, and publish findings transparently.",
  Publishing:
    "Our platform streamlines the publishing process, ensuring high-quality research reaches the right audience effectively.",
  Impact:
    "Published research contributes to global cybersecurity improvements and drives innovation in IT defense strategies.",
  "Action Steps":
    "Submit your research, collaborate with peers, and help companies secure their systems by providing actionable insights.",
} as const;
...
export default function Page() {
  const [selected, setSelected] =
    useState<keyof typeof summaryData>("Problem Space");
    ...
    <div className="p-[140px] px-[120px] flex flex-col gap-[40px]">
      <div className="flex flex-col gap-[64px]">
        <Image
          ...
          width={1440}
          height={300}
          className="absolute top-0 left-0 -z-1 bg-black opacity-1"
        />
        <div className="flex flex-col gap-[40px]">
          <h2 className="text-[64px]">How OpenQuanta Works</h2>
          <p className="text-[16px] opacity-64 w-[853px]">
            OpenQuanta is designed with a layered security model where no single
            component can independently authorize access to research content,
            contributor tools, or publishing workflows. Instead, access requests
            require coordination between multiple verification layers such as
            identity credentials, role-based permissions, and content-level
            smart contracts. SDKs and blah blah blah
          </p>
        </div>
      ...
      <section className="flex max-w-6xl mx-auto py-16 px-6 gap-10">
        ...
        <div className="w-1/4 border-r border-gray-700 pr-6">
          ...
          <ul className="space-y-3">
            {(Object.keys(summaryData) as Array<keyof typeof summaryData>).map(
              (key) => (
                <li
                  key={key}
                ...
                <button
                  ...
                  onClick={() =>
                    setSelectedIndex((prev) =>
                      Math.min(prev + 1, sidebarItems.length - 1)
                    )
                  }
                >
